...
if __name__ == "__main__":
    ...
    mz_login_account =get_dict('./mz_login_account.txt')
    mz_client_s.post( mz_client + '/Home/Public/login', data=mz_login_account)

    # # #  2、美住：创建美住客栈
    mz = meizhu.MeizhuClass(mz_client, mz_bpass)
    mz.mz_add_hotel(mz_client_s, mz_hotel_data)
    #
    # #  3、登陆bpass
    mz_bpass_s = requests.session()
    mz_bpass_s = mz.mz_login_bpass(mz_bpass_s)
    # # #
    # # 4、美住客栈通过审核
    mz.mz_bpass_pass_hotel(mz_bpass_s, h_name)
    # #
    # 5、美住新建房间
    mz.mz_add_room(mz_client_s)
    #
    #
    # # 6、登陆锁掌柜
    ...
    qrm_client_accout = get_dict('./qrm_client_account.txt')
    qrm_client_data = s.handle_hash(qrm_client_accout, hash)
    ...
    qrm.qrm_bpass_login(qrm_bpass_s, qrm_bpass_data)
