...
class UserCategoryDao:
    def __init__(self):
        self.db = DBUtil()
    def addUserCategory(self,user_id,daily_id,category_name):
        #USERID INT PRIMARY KEY,USERNAME VARCHAR(20),PASSWORD VARCHAR(32),REGTIME DATETIME,DELFLAG INT
        db = DBUtil()
        sql = 'insert into `USER_CATEGORY` VALUE(id,%d,%d,"%s")'%(user_id,daily_id,category_name)
    ...
    def getUserCategoryByName(self,user_login_id,categoryName):
        db = DBUtil()
        sql = "SELECT ID,CATEGORY_NAME FROM  `USER_CATEGORY` WHERE USER_LOGIN_ID =%s and CATEGORY_NAME='%s'" % (user_login_id,categoryName)
        data = db.execute(sql)
        if data:
            return True
        else:
