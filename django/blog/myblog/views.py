...
def register(req):
        if req.method == 'POST':
            ...
            if (uf.is_valid()):
                ...
                password = hashlib.md5(password.encode("utf-8")).hexdigest()
                UserInfo.objects.create(username=username, password=password,sex=sex,regtime=time)
...
def myblog(req):
    if req.method == "POST":
        ...
        if uf.is_valid():
            ...
            time = datetime.datetime.strptime(now_time, '%Y-%m-%d %H:%M:%S')
            # 逆序，查找最后一条userid=userid的数据
            Daily.objects.create(body=body, author_id=userid,title=title,created_time=time,category_id='1',modified_time= time)
        return  HttpResponseRedirect('/myblog/index/')
    else:
         uf =  DailyForm()
